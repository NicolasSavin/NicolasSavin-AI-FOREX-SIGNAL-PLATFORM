...
class SignalHubService:
    ...
    async def create_signal(self, payload: SignalCreateRequest) -> SignalCard:
        ...
        lifecycle_state = self._resolve_lifecycle_state(normalized_status, payload.state)
        ...
        signal = SignalCard(
            ...
            invalidation_ru="Сценарий отменяется при достижении стоп-лосса или ручном переводе статуса.",
    ...
    def _normalize_generated_signal(self, signal: dict, news_feed: list) -> SignalCard:
        ...
        lifecycle_state = self._resolve_lifecycle_state(normalized_status, signal.get("state"))
    ...
    @staticmethod
    def _resolve_lifecycle_state(status: SignalStatus, preferred_state: str | None = None) -> str:
        normalized = lifecycle_from_status(status).value
        if status == SignalStatus.ACTIVE and preferred_state in {"active", "open"}:
            ...
            annotations.extend(
                [
                    ...
                    ChartAnnotation(
                        ...
                        label="Поддержка" if side == "BUY" else "Сопротивление",
            ...
            annotations.extend(
                [
                    ...
                    ChartAnnotation(
                        ...
                        label="Сопротивление" if side == "BUY" else "Поддержка",
    ...
    def _build_default_levels(entry: float | None, stop_loss: float | None, take_profit: float | None) -> list[SignalLevel]:
        ...
        if entry is not None:
            levels.append(SignalLevel(label="Вход", value=entry, type="entry", description_ru="Базовая точка входа в сделку."))
        if stop_loss is not None:
            levels.append(SignalLevel(label="Стоп-лосс", value=stop_loss, type="stop_loss", description_ru="Защитный уровень отмены сценария."))
        if take_profit is not None:
            levels.append(SignalLevel(label="Тейк-профит", value=take_profit, type="take_profit", description_ru="Целевой уровень фиксации прибыли."))
        return levels
