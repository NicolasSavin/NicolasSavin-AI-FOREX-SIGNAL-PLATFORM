...
class TwelveDataWebSocketService:
    ...
    def __init__(self) -> None:
        ...
        self.heartbeat_seconds = float(os.getenv("TWELVEDATA_WS_HEARTBEAT_SECONDS", "10"))
        self.stale_after_seconds = float(os.getenv("TWELVEDATA_WS_STALE_AFTER_SECONDS", "30"))
        self.cooldown_seconds = float(os.getenv("TWELVEDATA_RATE_LIMIT_COOLDOWN_SECONDS", "900"))
    ...
    def get_all_prices(self) -> dict[str, Any]:
        return {
            ...
            "prices": {s: self.get_price(s) for s in self.symbols},
        }
    ...
    def _looks_like_rate_limit(payload: dict[str, Any]) -> bool:
        return TwelveDataWebSocketService._is_rate_limit_text(json.dumps(payload).lower())
    ...
    def _is_rate_limit_text(text: str) -> bool:
        ...
        return any(
            token in lowered
            for token in ("rate limit", "too many", "credits", "quota", "limit exceeded")
        )

    def _cooldown_until_utc(self) -> str | None:
        if self._cooldown_until <= time.time():
            return None
        return datetime.fromtimestamp(self._cooldown_until, tz=timezone.utc).isoformat()
