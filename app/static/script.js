...
function getImpactLabel(impact) {
  const labels = {
    high: 'Потенциально высокое влияние',
    medium: 'Потенциально среднее влияние',
    low: 'Потенциально низкое влияние',
...
function renderNews(rows, updatedAt, emptyTitle = 'Новостей пока нет', emptyText = 'Подтверждённые новости появятся здесь после обновления источника.') {
  ...
  rows.forEach((row) => {
    ...
    const title = document.createElement(row.link ? 'a' : 'p');
    ...
    title.textContent = row.title || 'Новость без заголовка';

    if (row.link) {
      title.href = row.link;
      title.target = '_blank';
      title.rel = 'noopener noreferrer';
      title.classList.add('news-card__link');
    }

    badge.className = `impact-badge impact-badge--${impact}`;
    badge.textContent = getImpactLabel(impact);

    text.className = 'news-card__text';
    text.textContent = row.description_ru || 'Описание отсутствует.';

    meta.className = 'news-card__meta';
    meta.textContent = `Источник: ${row.source || 'RSS'} • Опубликовано: ${formatUpdatedAt(row.published_at_utc)} • Оценка важности: эвристика`;
