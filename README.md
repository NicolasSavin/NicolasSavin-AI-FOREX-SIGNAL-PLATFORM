...
- Для live-сигналов добавлен deterministic генератор `backend/signal_text_generator.py`: `generate_signal_text(signal_data) -> str` формирует единый профессиональный абзац (контекст → причина входа → план → инвалидация) и пропускает отсутствующие данные без выдумывания.
