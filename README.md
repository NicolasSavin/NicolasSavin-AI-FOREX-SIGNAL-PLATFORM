...
- Narrative-генератор обновлён под формат одного аналитического абзаца (3–6 предложений): добавлена функция `generate_signal_text(signal_data)`, нормализация контекста `chart_patterns/harmonic_patterns`, приоритет confluence, явный торговый план (`entry/stop/tp`) и детерминированная invalidation-логика без выдумывания отсутствующих объёмов/дельты/опционов/фундамента.
...
### Пример формата full_text (до/после)
- До: несколько общих фраз с механическим перечислением trigger/confirmation, часто 6–8 предложений и блочный стиль.
- После: один цельный абзац 3–6 предложений, где последовательно описаны структура, confluence (SMC/ICT + chart/harmonic при наличии), причина входа, торговый план и условие отмены сценария.
- Пример (после): `EURUSD на H1 сохраняет шортовый сценарий: переход из range в нисходящий тренд после BOS вниз, снят buy-side liquidity над equal highs, order block: bearish OB 1.0900-1.0912. Confluence формируется через графический паттерн descending triangle, гармонический паттерн Bearish Gartley и волновую структуру: коррекционный откат в волну (2) завершён, поэтому вход рассматривается только внутри рабочей зоны, а не на случайном импульсе. Торговый план: вход 1.0900, стоп 1.0950, цель 1.0820, триггер — отказ от закрепления выше 1.0912 и CHOCH вниз на M15, стоп размещён за уровнем, где ломается структура и теряется сценарий, цель стоит у ближайшего внешнего пула ликвидности/ключевой зоны. После входа ожидается направленное движение вниз к 1.0820; сценарий отменяется при закреплении выше 1.0950.`
- Если конкретный слой данных отсутствует, он не добавляется в narrative: нет `delta/cumulative_delta/options/fundamental/patterns` в payload — нет соответствующих утверждений в тексте.
